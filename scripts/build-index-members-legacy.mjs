...
if (envCsv && fs.existsSync(envCsv)) {
  ...
  note =
    'ASX200 proxied from free BlackRock iShares IOZ ETF holdings (tracks S&P/ASX 200). ASX500/mid/small still weight-rank slices.'
} else {
  asx200 = ranked.slice(0, 200).map((r) => r.ticker)
  source = 'weight-rank-proxy'
  note =
    'All sets derived from weight ranks in asxUniverse.json. Run npm run refresh:index-members when EODHD token is set.'
}
